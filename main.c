...
int main(){
    ...
    int opt, rrn, n, i = 0, valorNroInscricao, rep = 0;
    int *rrns;
    ...
    case 5:
        ...
        while(i < n){
            ...
            if(i == n-1)
                remover_registro(nome_file, nome_campo, valor, nome_file2, rrns, OK); /* Função que remove registros que satisfaçam o critério de busca do usuario */
            else
                remover_registro(nome_file, nome_campo, valor, nome_file2, rrns, ERRO); /* Função que remove registros que satisfaçam o critério de busca do usuario */
            i++;
        }
        break;
    case 6:
    ...
    case 10:
        ...
        break;
    case 11:
        scanf("%s %s", nome_file, nome_file2); /* Digita o nome dos arquivos binários */
        cria_indice(nome_file, nome_file2);
        break;
    case 12:
        scanf("%s %s %s %[^\n]s", nome_file, nome_file2, campos, valor); /* Digita o nome dos arquivos binários */
        recupera_reg_ind(nome_file, nome_file2, valor);
        break;
    case 13:
        scanf("%s %s %d", nome_file, nome_file2, &n); /* Digita o nome do arquivo binario e o numero de iteracoes */
        while(i < n){
            scanf("%s %[^\n]s", nome_campo, valor); /* Digita o nome do campo e o criterio de busca */
            if(i == n-1)
                remover_registroInd(nome_file, nome_file2, nome_campo, valor, OK); /* Função que remove indice referentes à registros que satisfaçam o critério de busca do usuario */
            else
                remover_registroInd(nome_file, nome_file2, nome_campo, valor, ERRO); /* Função que remove indice referentes à registros que satisfaçam o critério de busca do usuario */
            i++;
        }
        break;
    }
