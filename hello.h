...
int ler_stringDel (FILE*, char*, char); /* Função que lê as strings até um delimitador */
...
int remover_registro(char*, char*, char*, char*, int*, int); /* Função que remove registros que satisfaçam o critério de busca do usuario */
...
void mergeSort(Dados_PTR*, int, int); /* Ordena o vetor com os registros */
...
void cria_indice(char*, char*); /* Função que cria um novo arquivo de indice */
void escreve_cabecalhoInd(FILE*, Header_ind); /* Função que escreve o cabecalho do arquivo de indice */
void escreve_DadosInd(FILE*, Dados_ind_PTR); /* Função que escreve os registros do arquivo de indice */
int cria_indDados(FILE*, Dados_ind_PTR, int); /* Função que obtem a chaveBusca e o rrn do arquivo de dados */
...
void recupera_reg_ind(char*, char*, char*); /* Função que exibe dados sobre registros a partir do arquivo de indice */
int ler_chave_ind(FILE*, char *); /* lê o campo chaveBusca até encontrar um \0 e retorna o numero de caracteres lidos */
void remover_registroInd(char*, char*, char*, char*, int); /* Função que remove indice referentes à registros que satisfaçam o critério de busca do usuario */
int binarySearch(Dados_ind_PTR, int, int, char*); /* Função que faz busca binária e retorna a posicao do valor encontrado */
